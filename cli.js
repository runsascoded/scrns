...
import { resolve as resolve2, basename, dirname as dirname2 } from "path";
...
function getScrnsRoot() {
  const root = resolve2(__dirname, "..");
  if (existsSync(resolve2(root, "package.json"))) return root;
  return root;
}
function getScrnsVersion() {
  const pkg = JSON.parse(readFileSync(resolve2(getScrnsRoot(), "package.json"), "utf8"));
  return pkg.version;
}
function packLocal() {
  const root = getScrnsRoot();
  const output = execFileSync2("npm", ["pack", "--pack-destination", "."], {
    cwd: root,
    encoding: "utf8"
  }).trim();
  const tarball = resolve2(root, output);
...
async function runInDocker(opts) {
  ...
  const outputDir = resolve2(opts.output);
  ...
  if (opts.config) {
    scrnsArgs.push("-c", `/work/${basename(resolve2(opts.config))}`);
  }
...
function findConfig(configOpt) {
  if (configOpt) {
    return resolve3(configOpt);
  }
  for (const name of DEFAULT_CONFIGS) {
    const path = resolve3(name);
    if (existsSync2(path)) {
      return path;
    }
  }
...
var previewCmd = program.command("preview [name]").alias("record").description("Open headful browser for interactive screenshot composition").option("--url <url>", "URL to open (overrides config entry)");
